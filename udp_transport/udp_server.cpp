...
void UdpServer::read_file_and_send(bool fin_flag, int start_byte,
  ...
  free(fileData);
  // `data_segment` was allocated with `new`, so use `delete` to release it.
  // Using `free` on memory allocated by `new` is undefined behavior and can
  // corrupt the heap (which could explain why the client blocks unpredictably).
  delete data_segment;
}
...
void UdpServer::send_data_segment(DataSegment *data_segment) {
  ...
  sendto(sockfd_, datagramChars, MAX_PACKET_SIZE, 0,
         (struct sockaddr *)&cli_address_, sizeof(cli_address_));
  // Do not free(datagramChars) here: DataSegment owns the internal buffer and
  // will free it in its destructor. Freeing it here causes a double-free.
}
}  // namespace safe_udp
